...
async function buildApiCompositionRoot(): Promise<ApiCompositionRoot> {
   ...
   *
   * ⚠️ **原注释说「生产方那一侧今天还没接线：自动化既没建中继、四个限频配置存储也都没传推进器」
   * ——整句都是过期说法，2026-08-05 逐处实核推翻**：
   *   - 四个配置存储（会话 / 配额 / 节奏 / 恢复）**都**接着自动化侧的 outbox 型推进器；
   *   - 中继**存在且在跑**（`aidcp-automation/src/automation-main.ts` 构造 `ConfigMirrorBumpRelay`、
   *     启动时 `start()`，写提交后 `wake()` 唤醒）。
   * 也就是说这条通道**两端都就位了**，不是「只有本端」。
   *
   * **留着这段话是因为它示范了一个真实代价**：那句「对面还没接」被原样转抄了不知多少轮，
   * 每转一轮就更像结论，直到有人真去 grep 才发现事实源早就变了。
   * **动手前先看事实源在谁那儿，别信任何一句转述的「对面拿不到」。**
   */
